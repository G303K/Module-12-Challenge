    ...
    .then((role) => {
      db.query("SELECT name, id FROM department", (err, res) => {
        ...
        inquirer
          ...
          .then((answer) => {
            const depID = res.filter(
              (data) => data.name === answer.department
            )[0].id;
            console.log(depID);
            db.query(
              "INSERT INTO role (title, salary, department_id) VALUES (?, ?, ?)",
              [role.name, role.salary, depID],
              (err, res) => {
                console.log(res);
                if (err) {
                  console.error(err);
                ...
                inquirer
                  .prompt({
                    ...
                    choices: employeeRes.map(
                      (role) => `${role.first_name} ${role.last_name}`
                    ),
                  })
                  .then((managerAnswer) => {
                    const { manager } = managerAnswer;
                    const managerId = employeeRes.filter(
                      (data) =>
                        `${data.first_name} ${data.last_name}` === manager
                    )[0].id;
                    db.query(
